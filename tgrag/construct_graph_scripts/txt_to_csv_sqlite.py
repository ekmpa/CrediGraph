...
def merge_vertices_rank_centrality(folder_path: str) -> None:
    ...
    if not os.path.exists(nodes_out_path):
        with (
            ...
            for vfile in vertex_files:
                ...
                with open(vfile, 'r') as vf:
                    for line in tqdm(
                        ...
                        cur.execute(
                            'SELECT pr_val, hc_val FROM ranks WHERE domain = ?',
                            (domain,),
                        )
                        result = cur.fetchone()
        ...
        logging.info('Skipping node CSV construction: file already exists.')

    if not os.path.exists(edges_out_path):
        with (
        ...
        logging.info('Skipping edge CSV construction: file already exists.')
