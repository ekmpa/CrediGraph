...
if [ -z "$SCRATCH" ]; then
    ...
    DATA_DIR="$SCRATCH" # was just $SCRATCH?
    echo "Using SCRATCH directory: $DATA_DIR"
fi

mkdir -p "$DATA_DIR/crawl-data"
...
if [ -n "$file" ]; then
  full_path="$DATA_DIR/crawl-data/$file"
...
while IFS= read -r wat_file; do
  echo "file:$DATA_DIR/crawl-data/$wat_file" >>"$input"
done <<< "$wat_files"
...
while IFS= read -r wat_file; do
  ...
  target_dir="$DATA_DIR/crawl-data/$first/$data_type/"
